...
Settings:
    ...
    * `AUDIENCE` - a list of authorized client IDs, defaults to [CLIENT]. Use this if you need to use both service and
                   bundle id to login both via iOS and from web forms.
    ...
    * `SOCIAL_AUTH_APPLE_ID_INACTIVE_USER_LOGIN` - allow users without confirmed email to login via this backend
"""
...
class AppleIdAuth(BaseOAuth2):
    ...
    def get_user_details(self, response):
        ...
        # prevent updating User with empty strings, as Apple only passes names on initial login for given account
        user_details = {
